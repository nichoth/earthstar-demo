...
console.log('keys', keypair1, keypair2)
// error could happen if our author shortname broke the rules
if (isErr(keypair1)) { throw "oops" }
if (isErr(keypair2)) { throw "oops" }
let author1 = keypair1.address
let author2 = keypair2.address

// You can set documents at specific paths, like a filesystem or
// key-value store.
...
console.log(storage.getContent('/wiki/Strawberry')) // --> 'Tasty'
...
console.log(storage.getContent('wiki/Strawberry')) // --> 'Yum'
...
console.log('history', storage.contents({
    path: '/wiki/Strawberry',
    includeHistory: true
}))
...
// Here we'll set the author's display name by writing to their profile
// document
storage.set(keypair1, {
    format: 'es.4',
